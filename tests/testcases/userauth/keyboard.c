...
void test_userauth_keyboard__interactive_auth_fails_with_wrong_response(void)
{
	...
	LIBSSH2_SESSION *session = cl_ssh2_connect_openssh_session((void *)WRONG_PASSWORD);

    cl_userauth_check_mech(session, USERNAME, "keyboard-interactive");
...
void test_userauth_keyboard__interactive_auth_succeeds_with_correct_response(void)
{
	...
	LIBSSH2_SESSION *session = cl_ssh2_connect_openssh_session((void *)PASSWORD);
