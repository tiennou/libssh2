...
void test_hostkey__initialize(void)
{
    session = cl_ssh2_connect_openssh_session(NULL);
}

void test_hostkey__cleanup(void)
{
    cl_ssh2_close_connected_session();
}

void test_hostkey__test_1(void)
{
    size_t len;
    int type;
    ...
    cl_assert_(hostkey != NULL, "libssh2_session_hostkey");
    ...
    if(type == LIBSSH2_HOSTKEY_TYPE_ECDSA_256) {
        ...
        cl_fail("Unexpected type of hostkey");
    }
    cl_assert_equal_i(expected_len, len);
    cl_assert_equal_i_(0, memcmp(hostkey, expected_hostkey, len), "Hostkeys do not match\n");
}
...
static void calculate_digest(const char *hash, size_t hash_len, char *buffer,
 ...
 void test_hostkey__test_2(void)
 {
     char buf[BUFSIZ];
     ...
     if(type == LIBSSH2_HOSTKEY_TYPE_ECDSA_256) {
         ...
         cl_assert_(md5_hash != NULL, "libssh2_hostkey_hash(LIBSSH2_HOSTKEY_HASH_MD5)");
         ...
         cl_assert_(sha1_hash != NULL, "libssh2_hostkey_hash(LIBSSH2_HOSTKEY_HASH_SHA1)");
         ...
         cl_assert_(sha256_hash != NULL, "libssh2_hostkey_hash(LIBSSH2_HOSTKEY_HASH_SHA256)");
